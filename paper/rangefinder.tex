...
hydride-on-member guard), a shadow equal-error range audit, and guarded
natural-isotope-envelope deconvolution for identifiable peak overlaps. The end-to-end model is intentionally
APT-specific: it assumes positive field-evaporated ions, APT peak shapes
and charge states, natural-isotope families, small inorganic molecular
ions, and elemental atomic percent as the target quantity. Because no two
open tools cover the same stage,
...
The end-to-end evaluation spans synthetic ground-truth specimens and eleven
public APT reference datasets covering pure metals, steels, alloys, an
oxide, and a nitride. Separate detector-only transfer checks use 120
MassBank-derived semi-synthetic spectra and real ToF-SIMS spectra; they do
not validate species assignment or composition outside APT. On the
complete-case APT benchmark, \rf{} has the lowest aggregate composition
error and mass error while retaining 1.000 intensity-weighted synthetic-line
recall and 0.984 precision; it processes a 7.7M-event specimen in under ten
seconds. All datasets, metrics, and the benchmark harness are released with
the code, and every number in this paper regenerates from the documented
commands.
\end{abstract}
...
because we could inspect its GPL source but could not execute the MATLAB
implementation or its interactive workflow without a MATLAB licence. That
proprietary dependency sits uneasily with the very FAIR
...
exactly zero, and every table in this paper regenerates from the documented
benchmark commands.
...
\node[stage] (detect) {multi-resolution detection\\ + optional rescue};
...
\node[stage, right=of assign] (out) {overlap deconvolution\\ + composition/audit};
\node[guard, below=of deblend] (recal) {local recalibration\\ completion + verification};
\node[guard, below=of family] (eer) {equal-error ranges\\ shadow audit};
...
\end{tikzpicture}
\end{adjustbox}
\caption{\rf{}'s APT-specific end-to-end stages. Event-level $m/z$ values
from \texttt{.pos}/\texttt{.epos} data flow left to right; the parsimony
guards (blue) are ablated individually in
Table~\ref{tab:ablation}.}
\label{fig:pipeline}
\end{figure}

\rf{} takes APT event data --- ordinarily big-endian
\texttt{.pos}/\texttt{.epos}, represented internally by event-level $m/z$
values and reconstructed positions --- and produces ranged peaks, species
assignments with per-candidate probabilities, elemental and isotopic
composition tables, audited isotope-ratio claims, and, when enabled, a
spatial homogeneity analysis. Low-level detection routines can be adapted
to other one-dimensional TOF event lists, but the family identification,
assignment, composition, isotope audit, and spatial stages described here
are APT-specific. Every threshold named below is a configuration key with
the quoted default.

\subsection{Multi-resolution detection with an optional log-domain channel}
Peaks are detected on histograms at several bin widths simultaneously
(0.01, 0.02, and 0.05\,Da candidates, capped at 0.02\,Da after a
Freedman--Diaconis-derived
maximum), and the per-width detections are merged with
prominence-weighted clustering. The default is a standard prominence
detector on Savitzky--Golay-smoothed counts. An optional
\emph{log-peakiness} channel subtracts a median-filter baseline in
$\log_{10}$ space and scores candidates by prominence over local MAD noise;
Table~\ref{tab:ablation} shows why it is disabled by default.
...
\subsection{Adaptive range audit}
The production area for an isolated peak is the nonnegative histogram
excess inside a symmetric window of at least 0.12\,Da half-width (capped at
0.18\,Da), with a linear baseline interpolated from neighboring sidebands;
accepted crowded-window fits replace these integrals. In parallel, \rf{}
computes a clean-room equal-error ranging (EER) estimate inspired by
Atom-Probe-Toolbox \citep{felfer_toolbox}. Between the peak apex and each
inter-peak valley, the inferred contribution of the peak is constrained to
decrease monotonically away from the apex. The boundary $b$ solves
\begin{equation}
S_{\mathrm{missed}}(b)=k B_{\mathrm{included}}(b), \qquad k=1,
\end{equation}
where the continuum follows the constant-TOF transformation
$B(m)=A/\sqrt{\max(m,1\,\mathrm{Da})}$ fitted through inter-peak bins.
Adjacent search intervals meet at their raw-count valley, so EER ranges
cannot overlap. Each peak records both boundaries and areas, estimated
purity, recovery, background counts, missed signal, counting uncertainty,
and the fixed-versus-EER discrepancy as a range-placement systematic.

EER is deliberately a \emph{shadow} estimate in the default pipeline. The
implementation ablation in Table~\ref{tab:quantification_ablation} found a
large benefit on the four synthetic spectra but material regressions on
seven of ten real controls. The diagnostics are therefore exposed for
falsification and future specimen-specific validation, but EER does not
silently replace the production area.
...
The assignment-probability composition first sums baseline-subtracted peak
areas over candidates, with a probability floor (5\%) and per-peak
renormalization so that losing candidates cannot siphon a few percent of
every strong peak into phantom elements. A parallel ``robust'' composition
uses only high-confidence rank-1 assignments.

\subsection{Guarded isotope-envelope overlap deconvolution}
Final quantification treats assignment and abundance estimation as separate
questions. Species with a high-confidence rank-1 anchor are expanded into
their enumerated natural-isotope envelopes, discarding components below
probability $10^{-5}$. A bipartite graph connects each
species to every detected peak that can contain one of its isotopologues;
connected components containing a shared peak are fitted independently by
the nonnegative Poisson model
\begin{equation}
\hat{\boldsymbol a}=\arg\min_{\boldsymbol a\geq0}
\sum_i\left[(F\boldsymbol a)_i-y_i\log(F\boldsymbol a)_i\right],
\end{equation}
where $F$ contains normalized natural-isotope fractions and $y_i$ is the
measured peak area. This is a clean-room adaptation of the constrained
overlap-quantification idea in Atom-Probe-Toolbox and AtomProbeLab
\citep{felfer_toolbox,london2019}, not copied code.

The fit is allowed to modify a peak allocation only when its envelope matrix
has at least as many measured peaks as species, full column rank, condition
number below $10^6$, at least 50\% observed envelope coverage for every
species, no more than eight coupled species, optimizer convergence, and
relative residual below 0.35. Otherwise the assignment probabilities pass
through unchanged. Accepted components report rank, condition number,
Poisson deviance, residual, fitted counts, and Fisher-information counting
uncertainties. The output tables retain the assignment-only composition in
\texttt{atomic\_percent\_assignment} beside the promoted deconvolved value,
so the correction is always reversible and auditable.
...
\textbf{Detector-only transfer data.} 120 MassBank records
\citep{massbank2024} --- curated, expert-annotated centroid lists for known
organic compounds --- are rendered into semi-synthetic event spectra with
the same peak-shape model as the synthetic APT data. Two real ToF-SIMS
spectra in a channel/mz/intensity text format \citep{zenodo15446699} are
converted to pseudo-events, with recall scored on an incomplete reference
list of known inorganic ions. For both studies we disable APT family hints
and element pruning, ignore assignment and composition outputs, and score
only detected peak positions. These checks probe whether the detector can
transfer to foreign line patterns and formats; they are not evidence that
the end-to-end pipeline generalizes beyond APT.
...
from the benchmark outputs by the bundled table-building script.

\input{tables/composition_table}

\input{tables/detection_table}

\input{tables/detection_ablation_table}

\input{tables/ablation_table}

\input{tables/quantification_ablation_table}

\input{tables/foreign_table}

\input{tables/runtime_table}

\subsection{Reading the results}
Table~\ref{tab:foreign} must be read narrowly. It shows that the detector
retains high intensity-weighted recall on APT-shaped MassBank renderings and
recovers every ion in the limited ToF-SIMS reference list. It does not test
negative-ion annotation, fragment chemistry, or non-APT composition, and
the ToF-SIMS list is too incomplete to establish detector precision.

Aggregated over the eleven datasets every method completed, \rf{}'s mean
composition error is 3.62\,at.\% L1 --- $2.5\times$ better than the best
comparator (PyCCAPT at 9.17, itself running \rf{}'s assignment stage) and
more than $7\times$ better than the independent quantifiers (APyT 27.56,
APAV 30.20). The pattern behind the mean matters more than the mean: on clean
...
while every other method reports between 44 and 140 L1 --- APAV and APyT
...
\subsection{Quantification ablation}
Table~\ref{tab:quantification_ablation} is the promotion test for the two
quantification changes. It holds peak detection and candidate generation
fixed and crosses fixed versus EER areas with assignment-probability versus
envelope-deconvolved allocation. Full EER lowers mean synthetic L1 from
0.768 to 0.182 (0.093 with deconvolution), but raises mean real-control L1
from 11.327 to 14.398: five controls improve, two are unchanged, and seven
regress, with the largest failures on FeO, the V--Al nitride, and two H/D
steels. That is evidence for reporting EER as an uncertainty channel, not
for making it the answer.

The guarded overlap solver is much more conservative. Nine of fourteen
scored controls are unchanged to 0.001\,at.\%, two improve, and three regress
by at most 0.303\,at.\% L1; mean L1 falls from 8.310 to 8.262. The largest
gain is the synthetic Al--Mg--Si coincidence (0.448 to 0.107), where the
isotope ladders identify the AlH$^+$/Si$^+$ allocation. The gain is small
enough that we retain the pre-deconvolution composition beside every result
and present this as a guarded correction, not a solved overlap problem.
...
Finally, the added audits remain inexpensive (Table~\ref{tab:runtime}). \rf{}
processes a 1.5M-event synthetic specimen in a median 3.4\,s and the
7.7M-event pure-W dataset in 9.7\,s single-core. Narrower detector or
quantifier stages such as pyOpenMS, APAV, and the na\"ive baseline can be
faster, but they do not perform the full unsupervised identification,
molecular-family recovery, overlap audit, and isotope analysis. The practical
gain came from removing two
...
\textbf{Scope and transferability.} \rf{} is an end-to-end APT ranging
system, not a general-purpose mass-spectrometry or spectroscopy framework.
Its current data model expects event-level $m/z$ and reconstructed
coordinates; its fitter assumes approximately constant width in
$t=\sqrt{m/z}$ and APT-like one-sided evaporation tails; and its assigner
searches positive atomic and small molecular ions using natural-isotope
patterns, APT charge-state priors, and field-evaporation-specific penalties.
It does not model negative-ion assignment, adduct and fragment chemistry,
tandem MS, retention time or ion mobility, arbitrary profile/centroid
inputs, or non-mass-spectrometry axes. The MassBank and ToF-SIMS experiments
exercise only the detector after adaptation, so their results must not be
read as end-to-end validation outside APT.

\textbf{Development-set contamination.} The controls in this paper were
used to decide whether EER and overlap deconvolution should be promoted, so
the ablation is development evidence rather than a held-out estimate. This
is especially important for the four synthetic spectra, whose background
model matches the model used by EER. We mitigate the risk by reporting all
per-dataset directions, retaining the legacy composition beside the new
one, refusing to promote EER after its real-data failures, and releasing the
entire harness for genuinely independent data.
...
\rf{} is an APT-specific, pure Python (NumPy/SciPy/pandas), MIT-licensed
pipeline with the
benchmark harness, synthetic generator, control-download script, and
this paper's table generator included. Reproduce the main APT benchmark by
running \path{scripts/download_benchmark_controls.sh}, followed by
\path{scripts/run_benchmark.py}. The MassBank, ToF-SIMS, and ablation studies
each have a dedicated script; the quantification promotion test is
\path{scripts/run_quantification_ablation.py}. Comparator versions and environments are
...
``Atom-Probe-Toolbox,'' GPLv3 software repository,
